...
Objects::Objects()
    : m_window{CreateWindow("Objects")}, m_device{CreateDevice(m_window)},
...
bool Objects::AppEvent(SDL_Event *event)
{
...
void Objects::CreateGraphicsPipeline()
{
...
void Objects::CreateSurfaceTexture()
{
    Noise noise{};
...
struct Corner
{
    Corner(float x, float y, float z, FXMVECTOR color) : X{x}, Y{y}, Z{z}, Color{color}
    {
    }

    PositionColorTextureVertex WithTexture(const Texture &t)
    {
        return PositionColorTextureVertex{X, Y, Z, Color, t.U, t.V};
    }
...
Objects::VertexBuffer Objects::CreateCubeVertexBuffer()
{
    ...
    Corner v0{-0.5, -0.5, -0.5, DirectX::Colors::Red};
    Corner v1{0.5, -0.5, -0.5, DirectX::Colors::Green};
    Corner v2{0.5, 0.5, -0.5, DirectX::Colors::Blue};
    Corner v3{-0.5, 0.5, -0.5, DirectX::Colors::Yellow};
    Corner v4{-0.5, -0.5, 0.5, DirectX::Colors::Red};
    Corner v5{0.5, -0.5, 0.5, DirectX::Colors::Green};
    Corner v6{0.5, 0.5, 0.5, DirectX::Colors::Blue};
    Corner v7{-0.5, 0.5, 0.5, DirectX::Colors::Yellow};

    // front
    vertexBuffer.Quad(v0.WithTexture(t0), v1.WithTexture(t1), v2.WithTexture(t2),
                      v3.WithTexture(t3));

    // bottom
    vertexBuffer.Quad(v4.WithTexture(t0), v5.WithTexture(t1), v1.WithTexture(t2),
                      v0.WithTexture(t3));

    // top
    vertexBuffer.Quad(v3.WithTexture(t0), v2.WithTexture(t1), v6.WithTexture(t2),
                      v7.WithTexture(t3));

    // right
    vertexBuffer.Quad(v1.WithTexture(t0), v5.WithTexture(t1), v6.WithTexture(t2),
                      v2.WithTexture(t3));

    // left
    vertexBuffer.Quad(v4.WithTexture(t0), v0.WithTexture(t1), v3.WithTexture(t2),
                      v7.WithTexture(t3));

    // back
    vertexBuffer.Quad(v5.WithTexture(t0), v4.WithTexture(t1), v7.WithTexture(t2),
                      v6.WithTexture(t3));
